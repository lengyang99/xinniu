...
export default class TableList extends PureComponent {
  ...
  getAllData(){
	  this.setState({
	    ...
	    queryStatistics({searchParams:JSON.stringify({isHistory:true})}).then(res => {
	      if (res.resultCode === 1000) {
          // console.log('xxxxxxxxxxxxxx',JSON.stringify(res.resultData));
	        this.setState({
  ...
  exportData=(flag)=>{
    ...
    if(flag){
      window.location.href = '/modules/manage/auditRoute/statisticsExport.htm?searchParams='+encodeURIComponent(JSON.stringify({isHistory:true}));
    }else{
      window.location.href = '/modules/manage/auditRoute/statisticsExport.htm?searchParams='+encodeURIComponent(params);
    }
  }
	      ...
	      <Col md={8} sm={12}>
	        ...
	        <Col md={8} sm={12}>
	            <span style={{ float: 'center', marginBottom: 24 }}>
	               ...
	               <Button  onClick={()=>this.exportData(false)}>导出</Button>
	             </span>
	         </Col>
        </Row>
      </Form>
    ...
    return (
     <PageHeaderLayout title="交租订单">
        ...
        <div className={styles.tableList}>
	      ...
	      <br/><br/><br/>  
	     <div className={styles.tableList}>
	     	<h4>历史数据&nbsp;<Tooltip placement="topLeft" title={"历史数据指到昨日的风控统计数据"}><Icon type="info-circle-o" /></Tooltip></h4>
          <Button  onClick={()=>this.exportData(true)}>导出</Button>
	        <Table
