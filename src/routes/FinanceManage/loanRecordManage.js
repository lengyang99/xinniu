...
} from '../../services/loanRecordmanage.js';
...
export default class TableList extends PureComponent {1
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'users/fetch',    //TODO:组件加载完成后，获取表格数据
      payload: {
        pageSize: 10,
        currentPage: 1,
        searchParams: ''
    ...
    loanRecordAmount({searchParams:this.state.formValues.searchParams}).then(res => {
    	if(res.resultCode == 1000){
    		this.setState({
    			loanRecordAmount:res.resultData
    		});
    	}
    });
  ...
  handleModalVisible = (flag = false,record={}) => {
    ...
    if(flag&&record.userId){
      //显示用户信息 - 弹框
      dispatch({
        type: 'users/userDetailfetch',
        payload: record.userId
      })
    }else{
      //关闭弹框
      dispatch({
        type: 'users/changeModal',
        payload: flag
      })
    }
  }
    ...
    return (
      <PageHeaderLayout title="用户信息">
        <Card bordered={false}>
          ...
          <div> 
          	<span>累计放款金额：{this.state.loanRecordAmount||0}元</span>
          </div>
