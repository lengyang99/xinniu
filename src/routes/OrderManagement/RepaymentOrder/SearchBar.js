...
export default Form.create()((props) => {
  const { form,handleSearch,handleFormReset,handleExport,bizStatus,offStatus} = props;
  ...
  const bizStatusOptions = bizStatus.map(item => <Option value={item.key} key={item.key}>{item.value}</Option>);
    ...
    <Form onSubmit={handleSearch} >
      ...
      <Col span={6}>
        ...
        <Col span={6}>
          <FormItem label="期数" {...layout}>
            {getFieldDecorator('schuduleNo', {rules: [{max: 14, message: '输入字符数不得超过14'}]})(
      ...
      <Row>
        ...
        <FormItem label="审核状态" {...layout}>
            {getFieldDecorator('offlineAuditStatus',{initialValue: '全部'})(
              <Select placeholder="请选择" >
                <Option value='全部' key='全部'>全部</Option>
                {offStatusOptions}
              </Select>
        ...
        <Col span={6}>
            ...
            <Col span={6}>
              <FormItem label="实际还款日" {...layout}>
                {getFieldDecorator('realRepayTime',{})(
                  <RangePicker />
                )}
              </FormItem>
            </Col>
